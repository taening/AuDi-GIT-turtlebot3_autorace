...
class ParkingDetector:
    ...
    def fn_find_dot(self, img_trans, direction='left'):
        ...
        for contour_dotlane in list_contour_dotlane:
            ...
            if (10 < w_dot < 40) and (20 < h_dot < 80):
                cv2.drawContours(img_debug, [contour_dotlane], 0, (0, 255, 0), 2)
                cv2.putText(img_debug, 'w: {}, h: {}'.format(w_dot, h_dot), (contour_dotlane[0][0][0]+10, contour_dotlane[0][0][1]+10), cv2.FONT_HERSHEY_SIMPLEX, 0.2, (0, 255, 255))
    ...
    def fn_find_stop(self, img_trans):
        # ROI 영역에 맞게 자른 이미지
        pers_height, pers_width = img_trans.shape[:2]  # shape is w384 x h240
        img_gray = cv2.cvtColor(img_trans[int(pers_height * 1/ 8):int(pers_height * 3/ 4), :].copy(), cv2.COLOR_RGB2GRAY)
        _, img_stop = cv2.threshold(img_gray, 200, 255, 0)
        img_stop = cv2.morphologyEx(img_stop, cv2.MORPH_CLOSE, np.ones((5, 5), np.uint8))
        ...
        for cnt in contours_stop:  # 점선 인식
            ...
            if 280 < w_stop:  # 주차공간 판단
                ...
                cv2.putText(img_debug, 'w: {}, h: {}'.format(w_stop, h_stop), (cnt[0][0][0]+10, cnt[0][0][1]+10), cv2.FONT_HERSHEY_SIMPLEX, 0.2, (0, 255, 255))
                stop_check = True
